...
function SimpleFooterAuth() {
  ...
  useEffect(() => {
    ...
    const currentSignInState = SimpleAuthService.isUserSignedIn();
    console.log("Footer Auth - Current sign-in state:", currentSignInState);
    setIsSignedIn(currentSignInState);
    
    if (currentSignInState) {
      const profile = SimpleAuthService.getUserProfile();
      console.log("Footer Auth - User profile:", profile);
      setUserProfile(profile);
    }
    
    // Set up auth change listener
    const unsubscribe = SimpleAuthService.onSignInChanged((isAuthenticated) => {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      await SimpleAuthService.signIn();
...
export default SimpleFooterAuth;
